...
const addressMatch = text.match(/(?:dia chi|DC|So nhà|đường|ở|tại)[:\s]+(.*?)(?=\s*ACTION|$)/i);

const address = addressMatch ? addressMatch[1].trim() : (session?.entities?.address || null);

// Cải tiến bắt cân nặng linh hoạt hơn
const weightMatch = text.match(/(\d+)\s*(kg|kí|ký|cân|nặng)/i) || text.match(/(?:nặng|bé)\s*(\d+)/i);

const nameMatch = text.match(/(?:tên|bé tên|bé tên là|con tên là)[:\s]*([a-zA-ZÀ-ỹ\s]+)/i);

// Bắt tuổi: Đọc cả số tuổi và đơn vị (nếu có)
const ageMatch = text.match(/(?:tuổi|bé được|mấy tuổi|tháng tuổi)[:\s]*(\d+)\s*(tuổi|tháng)?/i);
...
// Xử lý chuẩn hóa cân nặng
let extractedWeight = null;
if (weightMatch) {
    // Lấy số từ weightMatch (kiểm tra xem nhóm nào chứa chữ số)
    const weightStr = weightMatch[1] || weightMatch[2];
    const numberMatch = weightStr ? weightStr.match(/\d+/) : null;
    extractedWeight = numberMatch ? numberMatch[0] : null;
}

// Xử lý chuẩn hóa tuổi (lấy cả số và đơn vị)
let extractedAge = null;
if (ageMatch && ageMatch[1]) {
    extractedAge = `${ageMatch[1]}${ageMatch[2] ? ' ' + ageMatch[2] : ' tuổi'}`;
}
...
const entities = {
    ...
    weight: extractedWeight || (session?.entities?.weight || null),
    name: nameMatch ? nameMatch[1].trim() : (session?.entities?.name || null),
    age: extractedAge || (session?.entities?.age || null),
    group: foundGroup ? foundGroup : (session?.entities?.group || null) // Giữ group cũ nếu không tìm thấy cái mới
};
console.log("🎯 [DEBUG] Các biến nhận diện được từ tin nhắn:", JSON.stringify(entities, null, 2));
if (foundGroup) console.log(`🎯 [Analyzer] Đã xác định được Group: ${foundGroup}`);
        ...
        if (raw.includes("action_submit_form")) {
            zoneScores.CONVERSION = 999; // Điểm tối cao
            zoneScores.CONSULT = 0;
            zoneScores.RETAIN = 0;
            zoneScores.REMEDY = 0;
            console.log("🚀 [Analyzer] Phát hiện mã Form: Ép thắng CONVERSION");
        } else {
        ...
        if (entities.name) zoneScores.CONVERSION += 5;
        if (entities.age) zoneScores.CONVERSION += 5;
